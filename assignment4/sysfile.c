...
sys_open(void)
{
  ...
  if(omode & O_CREATE){
    ...
    if(ip == 0)
      ...
      if (omode & O_NODEREFERENCE){ // return 1 level
          char name[DIRSIZ];
          if((ip = namex(path, 0, name, 0, 0,1)) == 0) // namex will go dipper
              return -1;
      } else {
          if((ip = namei(path)) == 0)
              return -1;
...
//    cprintf("in open path: %s\n",path);
    ...
    // should check if inode protected

    // FIXME
    if(ip->type == T_DIR && omode != O_RDONLY
            && omode != (O_NODEREFERENCE | O_RDONLY)){
      iunlockput(ip);
      return -1;
    }
...
//    cprintf("pid: %d, ip->passwordSet: %d\n", proc->pid, ip->passwordSet);
    ...
    if(ip->type == T_FILE && ip->passwordSet && 
            !checkPremission(proc->pid, ip->inum)){
        iunlockput(ip);
        return -1;
    }
  }
